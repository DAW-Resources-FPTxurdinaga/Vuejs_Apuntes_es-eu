...
### Ejemplo 1: Solo con Props (Padre → Hijo)

En este ejemplo el componente hijo solo **muestra información** que recibe por props, sin emitir eventos.

**Componente Padre (UserProfilePage.vue):**
```vue
<template>
  <div>
    <h1>Perfil de Usuario</h1>
    <user-card :nombre="nombre" :edad="edad" :ciudad="ciudad" />
  </div>
</template>
...
const nombre = ref('Ane');
const edad = ref(28);
const ciudad = ref('Bilbao');
</script>
```
...
<script setup>
// Opción 1: definir props como array de nombres (forma abreviada)
// const props = defineProps(['nombre', 'edad', 'ciudad']);

// Opción 2: definir props como objeto (con tipo y validación)
...
### Ejemplo 2: Props + evento al padre (lista de tareas)
...
const emit = defineEmits(['tarea-completada']);
...
[→ Siguiente: Enrutamiento en Vue](../5-vue-router/5.1-introduccion.md)
