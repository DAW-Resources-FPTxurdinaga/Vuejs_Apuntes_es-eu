...
<template>
  <div>
    <!-- Campo con modificador .lazy para actualizar solo al perder el foco -->
    <div>
      <label>Nombre (lazy):</label>
      <input v-model.lazy="nombre" placeholder="Escribe tu nombre">
      <p>Nombre actualizado al perder foco: {{ nombre }}</p>
    </div>
    
    <!-- Campo con modificador .trim para eliminar espacios en blanco -->
    <div>
      <label>Usuario (trim):</label>
      <input v-model.trim="usuario" placeholder="Sin espacios al inicio/fin">
      <p>Usuario ({{ usuario.length }} caracteres): {{ usuario }}</p>
    </div>
    
    <!-- Campo con múltiples modificadores -->
    <div>
      <label>Búsqueda (lazy + trim):</label>
      <input v-model.lazy.trim="busqueda" placeholder="Escribe y pulsa tab">
      <p>Búsqueda actualizada: {{ busqueda }}</p>
    </div>
    ...
    <div>
      ...
      <label for="verificado">
        {{ verificado ? 'Activado' : 'Desactivado' }}
      </label>
    </div>
  </div>
</template>
...
[Siguiente: V-if y V-for →](v-if_v-for.md)
