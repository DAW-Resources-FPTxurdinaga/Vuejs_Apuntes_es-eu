...
### Ventajas del uso de querys

- **Compartir enlaces con el filtro aplicado**: la URL incluye los filtros (por ejemplo, `?category=books&page=2`), así que si copias y pegas el enlace, otra persona verá exactamente la misma vista.

- **El filtro persiste al refrescar la página**: como el estado va en la URL, al hacer F5 o recargar, Vue Router puede reconstruir los filtros a partir de la query.

- **Navegación atrás/adelante sin perder filtros**: cada cambio de filtro genera una nueva URL en el historial. Al darle a atrás/adelante, vuelves a cada estado anterior de filtros.

- **Puede ayudar al SEO (si la página es indexable)**: cada combinación de query puede ser una URL distinta que los buscadores pueden indexar, siempre que el contenido sea relevante.
